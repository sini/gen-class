...
#
# Every other cell here takes `genClassWithMerge` from `ci/flake.nix`, which builds it with
# `import ../lib` from ci's own flake INPUTS, so the root shim is never evaluated and its
# `ci/flake.lock`-backed defaults are never forced. That is precisely where this library's non-flake
# contract lives: `import ./. { }` must produce the same library the flake path does, resolving both
# dependencies from `./ci/flake.lock` with no argument supplied and no search path consulted.
#
# ★ THE COMPARISON TARGET IS `genClassWithMerge`, NOT `genClass` — a consequence of the owner ruling
# (arm A, `den-hoag-4dfsv`, 2026-09-15) rather than a choice made here: `merge` is now an ORDINARY
# formal, resolved the same three-channel way as `prelude`, so `import ./. { }` no longer defaults to
# `merge = null`. It defaults to the ci lock's own `gen-merge`, the same construction
# `genClassWithMerge` is built from. The two export the SAME NAMES either way (`AGENTS.md`'s drift
# check — `applyCoreFixed` is unconditionally exported and throws only when called with a null
# `merge`), so an `attrNames` comparison cannot itself discriminate which tier is meant; it is stated
# here because the VALUE now agrees with the tier-2 wiring, not because the cell below could tell.
#
...
let
  ...
  # last-segment shortcut reads a DIFFERENT node in general, and at THIS library's own ci lock the
  # two wired paths disagree on it: `gen-merge`'s node key equals its label, but `gen-prelude`'s does
...
in
{
  ...
  # The two entry paths are ONE library. `genClassWithMerge` is built from ci's flake inputs,
  # `dispatched` from the same `ci/flake.lock` read as data — so this compares the two suppliers of
  # one construction rather than an expression with itself, and NOT the root against `flake.lib`:
  # under L3 the flake output is `import ./. { prelude = gen-prelude.lib; }`, partially applied, so
  # ITS `merge` is the very same shim default this cell is exercising, and a cell built that way
  # cannot fail. Raw `dispatched == genClassWithMerge` is not the alternative either — `./lib`'s
  # surface carries function-valued members, and Nix `==` THROWS rather than reading false the
  # moment it descends into comparing two functions. `attrNames` compares only the published SURFACE
  # SHAPE the two suppliers agree on, which is both comparable and the thing actually worth asserting
  # here.
  flake.tests.entry.test-the-defaulted-entry-publishes-the-flake-surface = {
    expr = builtins.attrNames dispatched;
    expected = builtins.attrNames genClassWithMerge;
  };
  ...
  # this library needs at least at `prelude`, since ITS node key already differs from its label at
  # this repository's own real lock, while `merge`'s happens to agree and so cannot itself
  # discriminate. Replace the fold in `default.nix` with the shortcut and this reds, while the cell
  # above would stay green on the path where the two rules agree.

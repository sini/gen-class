...
# is evaluated by nothing. A shim can therefore name fewer arguments than the library it delegates
# to, promise lockstep with the flake in its own comment, and stay green forever: the flake path and
# the plain-import path are two signatures with nothing comparing them, so the second one rots
# silently the first time `lib/default.nix` grows a formal.
#
# ★★ THE CELL IS PURE, AND THE PURITY IS A CONSEQUENCE OF HOW IT IS CALLED. The shim's default
# `builtins.fetchTree`s the flake-locked rev; supplying the formal explicitly means that default is
# never forced, so this reaches the network not at all. What it tests is the shim's SIGNATURE and
# its delegation — which is precisely where the defect lives.
#
# ★ THE TIER-1 ARM IS THE ONE COMPARED, and that is the shim's own default rather than a choice this
# cell makes: `merge` is optional on both sides and defaults to null, so `import ../.. { prelude }`
# and the `genClass` the flake builds are the same construction reached two ways. The tier-2 surface
# is `applyCoreFixed`'s behaviour, which the apply-fixed suite owns; what is at stake here is the
# entry point, not the kernel.
#
...
#     (`called with unexpected argument 'prelude'`);
...
let
  ...
  entryArgs = {
    ...
    lock = { };
    # Declared `merge ? null`. Supplied as null, which is the same tier-1 construction this cell
    # already compared — the point is that the cell now CHOOSES it rather than inheriting a default
    # whose expression can change without notice.
    merge = null;
  };
...
in
{
  ...
  flake.tests.entry.test-standalone-entry-matches-lib = {
    ...
    expected = builtins.attrNames genClass;
  };
  ...
  flake.tests.entry.test-control-the-compared-surface-is-non-trivial = {
    expr = builtins.length (builtins.attrNames standalone) > 6;
    expected = true;
  };
  ...
  flake.tests.entry.test-control-the-comparison-discriminates = {
    expr = builtins.attrNames standalone == builtins.attrNames (removeAttrs genClass [ "mkClass" ]);
    expected = false;
  };

  # And the library reached THROUGH the shim actually works, rather than merely having the right
  # keys — a delegation that forwarded the wrong value would satisfy an `attrNames` check. The class
  # constructor validates its arguments inside `contract.nix`, which is built over the injected
  # `prelude`, so a shim forwarding something that is not gen-prelude fails here rather than passing
  # quietly.

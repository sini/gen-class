...
| Computing the partition key itself (classKey semantics — sorted include-sets / sanitised resolved-aspects) | **Nobody: `gen-resolve` was the source of the DISCIPLINE, never the owner of this key.** gen-class computes its own `classKey`; the sorted include-set / sanitised resolved-aspects discipline was borrowed from gen-resolve (`lib/partition.nix:3-12`) and is a citation of a retired library's law, not a live dependency — `git grep -n "gen-resolve" -- lib/` ⇒ no match (control, same command shape: `git grep -n -c "gen-merge" -- lib/` ⇒ `lib/apply.nix:5`, `lib/default.nix:1`). **`gen-resolve` DISSOLVED rather than moving as one library.** ADR-0008 §4 orphans the repository as reference and the 2026-09-02 transfer ruling dispositioned its eleven exports ROW BY ROW rather than as one content block (map at `den-hoag-p3y9`); take no new dependency on it |
...
| Cross-eval / cross-invocation incremental reuse (the deploy-time plane) | `gen-memo` — "gen-memo — the incremental plane: a decision layer over the evaluator that never evaluates, only decides reuse". README "Scope fences" states gen-class is intra-process only |
| Building NixOS systems, the nixpkgs boundary, value injection | The hub's `lib.compose` / interim `flakeModules.default` (INTERIM, not yet ADR-0027) — **`gen-flake` DISSOLVED rather than moving as one library.** ADR-0031 F2/F3 sent the compose S2 core to the hub, warm/override/trace to `gen-memo`, the projection + `realize` to `gen-delivery`, and inject/terminals to the crossing's Adapter set. The repo orphans as reference; take no new dependency on it |
